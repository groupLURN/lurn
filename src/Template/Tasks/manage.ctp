<?= $this->assign('title', 'Task Management') ?>
                            ...
                            <?php foreach ($milestone->tasks as $task): ?>
                                <tr>
                                    ...
                                    <td>
                                        <div class="progress">
                                            <div class="progress-bar progress-bar-info progress-bar-striped" role="progressbar"
                                                 aria-valuenow="<?=isset($task->replenishment)?$task->replenishment: 0 ?>"
                                                 aria-valuemin="0" aria-valuemax="100" style="margin-right: -<?=isset($task->replenishment)?$task->replenishment: 0 ?>%;
                                                width: <?=isset($task->replenishment)?$task->replenishment: 0 ?>%">
                                            </div>
                                            <div style="text-align:center; color:black;"><?= h(number_format(isset($task->replenishment)?$task->replenishment: 0, 2)).'% Complete'?></div>
                                        </div>
                                    </td>
                                    <td>
                                    ...
                                    <td class="actions">
                                        ...
                                        <?= $this->dataTableManageButton(__('Finish'), ['action' => 'finish', $task->id, '?' => ['project_id' => $projectId]]); ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                            </tbody>
