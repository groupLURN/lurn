$(function(){
	...
	$('#from-project-id').chosen().change(function() {
		...
		$.ajax({ 
			type: 'GET', 
			url: link+'resource-request-headers/get-milestones?project_id='+projectId, 
	...
	$('#milestone-id').chosen().change(function() {
		...
		$.ajax({ 
			type: 'GET', 
			url: link+'resource-request--headers/get-tasks?project_id='+projectId+'&milestone_id='+milestoneId, 
			data: { get_param: 'value' }, 
			success: function (data) { 
				var tasks = data;
